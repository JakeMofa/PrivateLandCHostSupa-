...
export default function AdminListingReviews({ onLogout }: AdminListingReviewsProps) {
  ...
  const loadListings = async () => {
    try {
      ...
      console.log('🔍 AdminListingReviews: Loading listings from database...');
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          broker:users!broker_id(id, first_name, last_name, email, phone),
      ...
      let mockListings = (data || []).map((listing: any) => {
        const brokerFullName = listing.broker 
        ...
        return {
          ...
          broker: listing.broker ? {
            full_name: brokerFullName,
            email: listing.broker.email,
            phone: listing.broker.phone || 'N/A',
            license_number: 'On file',
            brokerage: 'Brokerage on file'
          } : null,
          // Parse JSON fields if they're strings
          features: Array.isArray(listing.features) ? listing.features : (listing.features ? JSON.parse(listing.features) : []),
          ...
          city: listing.city || listing.county || 'Unknown',
          property_type: listing.property_type || 'Other',
          // Map consent status (approved -> verified for UI)
          client_consent: listing.client_consent ? {
            ...listing.client_consent,
            status: listing.client_consent.status === 'approved' ? 'verified' : listing.client_consent.status
          } : null
        };
      });
      
      console.log('📝 Total listings from database:', mockListings.length);
      
      // NOTE: mockListings now contains real data transformed from database
      // Remove the static mock array below this line:
      const _unusedMockData = [
        {
          id: '1',
          title: 'Highland Ranch Estate',
          property_type: 'Ranch',
